...
Jmat =   np.array([[-0.52, 0   ,     0],
                   [0    , 0.52,     0],
                   [0    ,    0, -0.72]])

Jmatrotx=np.array([[-0.52,     0,     0],
                   [0    , -0.72,     0],
                   [0    ,     0,  0.52]])


_,_,Jp12=Jprime(Jmat,S1,S2)
_,_,Jp34=Jprime(Jmat,S3,S4)
_,_,Jp56=Jprime(Jmatrotx,S5,S6)
_,_,Jp78=Jprime(Jmatrotx,S7,S8)
...
for h in np.arange(0.1,4,0.5):
    for i in range(0,50):
        ...
        res = minimize(FEnergy,initial,bounds=bnds,tol=1e-7)
        print(h,i)
        if res.fun<=E:
            new_res=res
            E=res.fun
...
ax[3,0].plot(field,energy,'.-')
ax[3,0].set_title("System Energy")

ax[3,1].plot(field, (np.abs(bulk_m))**2,field, 4*(np.array(stagger_m))**2)
ax[3,1].set_title("Overlay Stagger and Bulk Magnetization")